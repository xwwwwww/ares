...
class ODIAutoPGDAttacker(BatchAttack):
    ...
    def init_auto(self):
        ...
        self.config_fmax = self.fmax.assign(self.fmax_ph)
        self.config_xmax = self.xmax.assign(self.fmax_ph)
    ...
    def batch_attack(self, xs, ys=None, ys_target=None):
        ...
        for i in range(self.Nr):
            ...
            if f0.mean() >= f1.mean():
                ...
                # self._session.run(op)
                # op = xmax.assign(x0)
                self._session.run([self.config_fmax, self.config_xmax], feed_dict={self.fmax_ph: f0, self.xmax_ph: x0})
            else:
                ...
                # op = xmax.assign(x1)
                # self._session.run(op)
                self._session.run([self.config_fmax, self.config_xmax], feed_dict={self.fmax_ph: f1, self.xmax_ph: x1})
                fcnt += 1
